...
function insertData($tableName, $data) {
    ...
    if ($conn->query($sql) === TRUE) {
        if($tableName === 'step')
        {
            updateWorkFlowLength($data['workflow_id_']);
        }
...
function updateWorkFlowLength($workflow_id_)
{
    // print($workflow_id_);
    $workflow_id_ = (int)$workflow_id_;
    global $conn;
    $tupleCount = countOfRowsInTable('step',"workflow_id_ = '$workflow_id_'");
    $data = array();
    $data['workflow_step_len'] = $tupleCount;

    // updateData($conn, $tableName, $data, $condition)
    updateData("workflow", $data,"workflow_id_ = '$workflow_id_'");
    // print($tupleCount);
}
...
// deleteData('step',"workflow_id_ = 1");

// updateData('step',$step_data,"step_id_ = '1'");
$conn->close();
?>
